...
> Multi-Coach Mode. Huyang reads `coach-kaytoo/SKILL.md` and channels K-2SO's voice for
> the duration of the turn. The schedules and message bodies are otherwise unchanged.
> Each `--message` is prefixed with
> `Channel K-2SO (🪞) — read coach-kaytoo/SKILL.md and run this in K-2SO's voice. ...`.
