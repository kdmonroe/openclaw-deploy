...
```json5
{
  agents: {
    list: [
      ...
      {
        id: "ops",
        model: { primary: "anthropic/claude-sonnet-4-5", fallbacks: ["zai/glm-5"] },
...
> **MEMORY.md warning (from docs):** _"Keep them concise — especially MEMORY.md, which can grow over time and lead to unexpectedly high context usage and more frequent compaction."_ MEMORY.md is the most common source of bootstrap bloat. Unlike AGENTS.md or SOUL.md which users actively edit, MEMORY.md tends to grow unchecked as the agent appends to it.
...
**Root cause:** `launchctl setenv` sets variables at the launchd _domain_ level, independent of any plist. These persist until the user logs out or they are explicitly unset. `kickstart -k` re-reads the plist for `ProgramArguments` and `EnvironmentVariables`, but the domain-level env set by `setenv` takes precedence.
