...
> Multi-Coach Mode. Huyang reads `coach-threepio/SKILL.md` and channels C-3PO's voice
> for the duration of the turn. The schedules and message bodies are otherwise unchanged.
> Each `--message` is prefixed with
> `Channel C-3PO (💰) — read coach-threepio/SKILL.md and run this in C-3PO's voice. ...`.
